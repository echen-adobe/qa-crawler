...
def exact_match_urls(block_map: Dict[str, Any], tokens: List[str]) -> List[str]:
    ...
    for _hash_key, data in block_map.items():
        ...
        # Exact means every token appears as its own class name
...
def top_similar_combinations(block_map: Dict[str, Any], query: str, top_k: int = 5) -> List[Tuple[str, int, str]]:
    # Build list of unique class name combinations (sorted, space-joined)
    combos: List[str] = []
    keys: List[str] = []
    ...
    # results: List[Tuple[str combo, score, index]]
...
def main() -> int:
    ...
    # Resolve path: --path > output/<date>/block_map.json > latest output/*/block_map.json > backend/qa/block_map.json
    if args.path:
        resolved_path = args.path
    else:
        repo_root = Path(__file__).resolve().parents[1]  # qa-crawler/
        output_dir = repo_root / "output"
        candidate: Optional[Path] = None
        date_pattern = re.compile(r"^\d{4}-\d{2}-\d{2}$")

        if args.date:
            candidate = output_dir / args.date / "block_map.json"
